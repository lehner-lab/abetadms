...
#' @param width plot width in "units" (default:8)
...
#' @export
#' @import data.table
abetadms__single_mutant_heatmap<-function(
  input_df, 
  ...
  if(!is.null(mut_dict)){
    for(mut_letter in names(mut_dict)){
      for(mut in mut_dict[[mut_letter]]){
        if(gsub("[A-Z]", "", mut) %in% colnames(heat_mat_text)){
          heat_mat_text[substr(mut, nchar(mut), nchar(mut)),gsub("[A-Z]", "", mut)] <- paste0(heat_mat_text[substr(mut, nchar(mut), nchar(mut)),gsub("[A-Z]", "", mut)], mut_letter)
        }
      }
    }
  }
  ...
  abetadms__tile_heatmap_wrapper(
    ...
    xlab = "Aß(1-42) amino acid position", 
    ylab = "Mutant AA", 
    cluster = cluster, 
    width = width, 
    height = height, 
