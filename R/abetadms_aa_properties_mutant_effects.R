...
#' abetadms_aa_properties_mutant_effects
#'
...
#' @export
#' @import data.table
abetadms_aa_properties_mutant_effects <- function(
  fitness_dt,
  outpath,
  aaprop_file,
  ...
  execute = TRUE
	...
	#Display status
	message(paste("\n\n*******", "running stage: abetadms_aa_properties_mutant_effects", "*******\n\n"))
  ...
  top5pc_var <- sum((exp_pca$sdev^2/sum(exp_pca$sdev^2))[1:5])
	...
	singles_dt <- abetadms__aa_properties_pca_singles_loadings(
		input_dt = singles_dt, 
		aa_properties_file = aaprop_file, 
		selected_identifiers = unlist(fread(aaprop_file_selected, header = F)))
	...
	#Add amino acid properties
	doubles_dt <- abetadms__aa_properties_pca_doubles_loadings(
		input_dt = doubles_dt, 
		aa_properties_file = aaprop_file, 
		selected_identifiers = unlist(fread(aaprop_file_selected, header = F)))
	...
	# mean_fitness <- singles_dt_aaprop[STOP==F,mean(abs(fitness))]
	# Pos_abs_hotspot <- singles_dt_aaprop[Nmut_aa==1 & !STOP,.(hotspot = mean(abs(fitness))>mean_fitness),by=Pos_abs][hotspot==T,Pos_abs]
	# singles_dt_aaprop[, hotspot := as.numeric(Pos_abs %in% Pos_abs_hotspot)*2]

	# #Single mutant AA properties
	# singles_dt_aaprop <- abetadms__aa_properties_singles_loadings(
	# 	input_dt = singles_dt_aaprop,
 #  	aa_properties_file = aaprop_file, 
 #  	selected_identifiers = unlist(fread(aaprop_file_selected, header = F)))

	# #Single mutants in hotspot hydrophobicity scales
	# singles_dt_hydro <- singles_dt_aaprop[hotspot == 2,.SD,,.SDcols = c("fitness", paste0(names(aa_evidences)[feature_type=="1_hydrophobic/Hydrophobic"], "_score"))]
	# names(singles_dt_hydro) <- gsub("_score$", "", names(singles_dt_hydro))

	# #Barplot of R-squared for all scales
	# plot_df <- data.frame(
	# 	r_squared = c(cor(singles_dt_hydro)[1,-1]^2, singles_dt_aaprop[hotspot == 2,cor(.SD[[1]], .SD[[2]])^2,,.SDcols = c("fitness", "PC1 (Hydrophobicity)")]),
 ...
 #    ggplot2::theme(axis.text.x=ggplot2::element_text(angle = 90, hjust = 1, vjust = 0.5))
	# ggplot2::ggsave(file=file.path(outpath, 'hydrophobicity_scales_correlations.pdf'), width=5, height=10, useDingbats=FALSE)
