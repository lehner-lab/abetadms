...
#' @export
#' @import data.table
abetadms_wt_helix_propensity <- function(
  fitness_dt,
  outpath,
  aaprop_file,
  ...
  #Display status
  message(paste("\n\n*******", "running stage: abetadms_wt_helix_propensity", "*******\n\n"))
	...
	#PCA
  exp_pca <- abetadms__aa_properties_pca(aa_properties_file = aaprop_file, selected_identifiers = unlist(fread(aaprop_file_selected, header = F)))
	pc_mat <- exp_pca$x
	...
	ggplot2::ggsave(file=file.path(outpath, 'tdp43_wt_pc_loadings.pdf'), width=14, height=40, useDingbats=FALSE)

	#WT loadings - hydrophobicity
	singles_df <- singles_dt[,.(position = Pos_abs, value = abs(fitness)*10-15, variable = "fitness")]
	plot_df <- plot_df[grepl("PC2 ", plot_df[,"variable"]),]
	hydro_min <- min(plot_df[,"value"])+0.2*min(plot_df[,"value"])
	hydro_max <- max(plot_df[,"value"])+0.2*max(plot_df[,"value"])
	#Test correlation between wt hydrophobicity and mean absolute fitness per position
	plot_df_test <- plot_df[plot_df[,"position"] %in% Pos_abs_all & grepl("PC2 ", plot_df[,"variable"]),]
	test_dt <- singles_dt[,.(mean_tox = mean(abs(fitness))),by=Pos_abs]
	test_dt[, wt_helix_propensity := plot_df_test[as.character(test_dt[,Pos_abs]),"value"]]
	tox_cor <- test_dt[,.(R = cor(wt_helix_propensity, mean_tox), n = .N)]
	d <- ggplot2::ggplot(plot_df, ggplot2::aes(position, value, color=variable)) +
  ...
  if(!is.null(colour_scheme)){
	...
	ggplot2::ggsave(file=file.path(outpath, 'tdp43_wt_helix_propensity_loadings.pdf'), width=8, height=3, useDingbats=FALSE)

	save(singles_dt, singles_df, plot_df, file = file.path(outpath, 'abetadms_wt_helix_propensity.RData'))
}
